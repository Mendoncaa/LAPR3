...
cursor c_campos is select distinct c.quantidade, k.designacao, k.hectares, e.nome from accao a 
...
open c_campos;
    ...
    DBMS_OUTPUT.PUT_LINE('Designacao -- Quantidade/Hectares -- Especie vegetal');
    dbms_output.put_line(chr(0));
    loop 
        fetch c_campos into v_campos;
        exit when c_campos%NOTFOUND;
        DBMS_OUTPUT.PUT_LINE(v_campos.designacao||' -- '||v_campos.quantidade/v_campos.hectares||' -- '||v_campos.nome);
    end loop;
    close c_campos;
        else
        dbms_output.put_line(chr(0));
