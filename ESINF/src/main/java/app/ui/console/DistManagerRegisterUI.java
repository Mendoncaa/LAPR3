...
public class DistManagerRegisterUI implements Runnable{

    private RegisterUserController ctrl;

    public DistManagerRegisterUI() {
    ...
    public void run() {
        ...
        do {
            ...
            try {
                ...
                ShowTextUI show = new ShowTextUI("###        Distribution Manager        ###");   //show this title
                show.run();
        ...
        boolean newEmployee = this.ctrl.createNewUser(name, email, Constants.ROLE_DISTRIBUTION_MANAGER); //creation object confirmation

        if (newEmployee) { // if the program has the confirmation that was created an employee
            this.ctrl.saveUser(); // saves it
            ShowTextUI success = new ShowTextUI("New distribution manager successfully added!"); // and show this message
            success.run();
        } else {
