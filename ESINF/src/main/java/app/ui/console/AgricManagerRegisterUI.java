...
public class AgricManagerRegisterUI implements Runnable {


    private RegisterUserController ctrl;

    public AgricManagerRegisterUI() {
    ...
    public void run() {
        ...
        do {
            ...
            try {
                ...
                ShowTextUI show = new ShowTextUI("###        Agricultural Manager        ###");   //show this title
                show.run();
        ...
        boolean newEmployee = this.ctrl.createNewUser(name, email, Constants.ROLE_AGRICULTURAL_MANAGER); //creation object confirmation

        if (newEmployee) { // if the program has the confirmation that was created an employee
            this.ctrl.saveUser(); // saves it
            ShowTextUI success = new ShowTextUI("New Agricultural Manager successfully added!"); // and show this message
            success.run();
        } else {
